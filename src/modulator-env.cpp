...
float ADSREnvelope::modulate_filter_q(double input, KeyStatus status, double time, float start_level){
    return input * adsr.get_level(status,time,start_level);
};

float ADSREnvelope::modulate_filter_q(double input){
    return modulate_filter_q(input,KEY_OFF,0.0,0.0);
};

float ADSREnvelope::modulate_filter_fc(double input, KeyStatus status, double time, float start_level){
    return input * adsr.get_level(status,time,start_level);
};  
float ADSREnvelope::modulate_filter_fc(double input){
    return modulate_filter_fc(input,KEY_OFF,0.0,0.0);
};
